...
# Adapted from: https://github.com/ITSLeeds/jts/blob/master/data-raw/jts_tables.R

library(rvest)
library(tidyverse)


# Bulk download ods files -------------------------------------------------
...
system("xlsx2csv -a jts0102.xlsx jts1010-xlsxcsv") # works, duplicate file names present...
...
for(i in 1:length(xlsx_files)) {
  xlsx_file_name = xlsx_files[i]
  folder_name = gsub(pattern = ".xlsx", "", xlsx_file_name)
...
f = list.files("jts_csv_files")
setwd("jts_csv_files")
piggyback::pb_upload(f, repo = "datasciencecampus/jtstats", tag = "0")
length(f)
setwd(old_directory)
